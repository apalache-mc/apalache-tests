...
/usr/bin/time -f 'elapsed_sec: %e maxresident_kb: %M' -o time.out /usr/bin/timeout --foreground 2h /home/runner/work/apalache-tests/apalache-tests/_apalache/apalache-0.17.6-SNAPSHOT/bin/apalache-mc check --profiling=true --run-dir=./out --init=Init --next=Next --inv=Inv --smt-encoding=arrays --length=10 --cinit=CInit10  SetSndRcv.tla | tee apalache.out
exitcode="$?"
