...
\* We do not support integers ranges with non-constant arguments.
\* However, it is easy to implement them with constant bounds.
BallotRange(a, b) == { i \in Ballot \cup {None} : a <= i /\ i <= b }

(*
ConstInit3 ==
    /\ Value \in {{0, 1}}
    /\ Ballot \in {0..2}
    /\ Acceptor \in {{"a1", "a2", "a3"}}
    /\ Quorum \in {{{"a1", "a2"}, {"a2", "a3"}, {"a1", "a3"}}}

ConstInit5 ==
    /\ Value \in {{0, 1}}
    /\ Ballot \in {0..3}
    /\ Acceptor \in {{"a1", "a2", "a3", "a4", "a5"}}
    /\ Quorum \in {{ \* script-generated
        {"a1", "a2", "a3"},
        {"a1", "a2", "a4"},
        {"a1", "a3", "a4"},
        {"a2", "a3", "a4"},
...
*)
-----------------------------------------------------------------------------
\* APALACHE: as we do not support instances yet, expand the definitions
...
ShowsSafeAt(Q, b, v) == 
  ...
  /\ \E c \in BallotRange(-1, b-1) (*-1..(b-1)*) : 
      /\ (c # -1) => \E a \in Q : VotedFor(a, c, v)
      /\ \A d \in BallotRange(c+1, b-1) (*(c+1)..(b-1)*), a \in Q : DidNotVoteAt(a, d)
...
SafeAt(b, v) == \A c \in BallotRange(0, b-1) (*0..(b-1)*) : NoneOtherChoosableAt(c, v)

VotesSafe == \A a \in Acceptor, b \in Ballot, v \in Value :
                 VotedFor(a, b, v) => SafeAt(b, v)
       
VotingInv == VotesSafe /\ OneValuePerBallot
