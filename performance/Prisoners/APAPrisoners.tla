...
\* same rewritten as an invariant
SafetyInv == Done => (\A i \in Prisoner \ {Counter} : timesSwitched[i] > 0)
...
\* APALACHE: instead of the recursive definition of sum, we give a declarative spec.
\* Here we assume that Prisoner is a set of integers, which makes specification simpler.
IsSum(no) ==
    \* there is a bijection between OtherPrisoner and the numbers 1..Cardinality(OtherPrisoner)  
    \E pr \in [1..Cardinality(OtherPrisoner) -> OtherPrisoner]:
        /\ \A i, j \in DOMAIN pr: pr[i] = pr[j] => i = j
        \* there is a recursive scheme:
        \* 1. sum[1] = timesSwitched[pr[1]] and
        \* 2. sum[i] = sum[i - 1] + timesSwitched[pr[i]] for i >= 2  
        /\ \E sum \in [DOMAIN pr -> 0..2 * Cardinality(Prisoner)]:
            /\ sum[1] = timesSwitched[pr[1]]
            /\ \A i \in DOMAIN pr \ {1}:
                sum[i] = sum[i - 1] + timesSwitched[pr[i]]
            /\ no = sum[Cardinality(OtherPrisoner)]

CountInv2 ==
    LET oneIfUp == IF switchAUp THEN 1 ELSE 0 IN
        \E x \in {0, 1}:
            IsSum(count + oneIfUp - x)


\* APALACHE: this invariant is using a recursive function, which we do not support
(*
CountInvariant ==
  (*************************************************************************)
